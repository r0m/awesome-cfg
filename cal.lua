...
function displayMonth(month,year,weekStart)
	...
	lines = lines .. "\n" .. os.date(" <span color='#0f5b85'>%V</span>",os.time{year=year,month=month,day=1})
	...
	while writeLine < (stDay + 1) do
        ...
        for d=1,mthDays do
                ...
                if writeLine == 8 then
                        writeLine = 1
                        lines = lines .. "\n" .. os.date(" <span color='#0f5b85'>%V</span>",t)
                end
                if os.date("%Y-%m-%d") == os.date("%Y-%m-%d", t) then
                        x = string.format(current_day_format, d)
                end
		if os.date("%w", t) == "0" then
		   x = string.format("<span color='#e63e3e'>%s</span>", d)
                end
...
function cal.register(mywidget, custom_current_day_format)
	...
	awful.button({ }, 2, function() goToToday() end),
